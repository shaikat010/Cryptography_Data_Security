...
def test()-> None:
    item_no = input("Put the number of items ypu want to put in the merkle tree: ")
    item_no = int(item_no)
    elems = []
    while(item_no!= 0):
        element = input("Please give me the element to put in the merkle tree: ")
        elems.append(str(element))
        print("THis is the list now: " + str(elems))
        item_no = item_no -1
